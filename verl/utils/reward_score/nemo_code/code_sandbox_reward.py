...
def execute_stdin_tests(
    ...
    test_inputs = []
    test_outputs = []
    test_inputs = cc_tests['input']
    test_outputs = cc_tests['output']
...
def compute_reward(config, problem_type, all_formatting, all_correct_lists, all_num_reasoning_tokens, all_num_non_reasoning_tokens, event):
    ...
    overlong_len_threshold = int(reward_fn_args.overlong.threshold_factor * max_gen_len)
    overlong_penalty_factor = reward_fn_args.penalties.overlong
    ...
    if event != 'val':
        overthreshold_answer = [
            max(0, (all_num_reasoning_tokens[i] + all_num_non_reasoning_tokens[i] - overlong_len_threshold) / (max_gen_len - overlong_len_threshold))
            for i in range(n)
        ]
        ...
        all_penalties = [p + (o if correct else u) for p, u, o, correct in zip(all_penalties, unused_reasoning_penalty_vals, overthreshold_answer_penalty, pass_or_not)]
...
def convert_functional_tests_to_asserts(func_name, tests):
    unit_tests = []
    outputs = set(tests['output'])

    boolean_output = False
    if outputs.issubset({'true', 'false'}) or outputs.issubset({'True', 'False'}) or outputs.issubset({True, False}):
        boolean_output = True
...
class RewardManager:
    ...
    def __call__(
        ...
        pass_tensor = torch.zeros(reward_tensor.shape[0], device=reward_tensor.device, dtype=torch.int32)
        reward_vector = torch.zeros(reward_tensor.shape[0], device=reward_tensor.device, dtype=torch.float32)
